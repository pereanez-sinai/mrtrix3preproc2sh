...
with open('./output/stdout.txt', 'w') as stdout, open('./output/stderr.txt', 'w') as stderr:
    ...
    # Do cat and preproc
    catname = os.path.join('.', 'output', f'dwi_{list(phase_enc_assign.keys())[0]}_{max(bmax_assign.values())}+{len(bmax_assign.values())}sh.mif')
    ...
    proc_cmds.append(f'dwifslpreproc {catname} {dwipreproc_output} -rpe_header -force {se_epi_str} -scratch {output_dir} -debug -topup_options " --verbose " -eddy_options " {eddy_speed_params} " 1> ./output/stdout.txt 2> ./output/stderr.txt')
    ...
    proc_cmds.append(f'dwibiascorrect fsl {dwipreproc_output} {bias_dwipreproc_output_nii}.gz -mask {fslmask_output}_mask.nii.gz -force')
